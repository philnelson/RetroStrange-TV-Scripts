...
for media_type in generator_config['types']:
    ...
    if not generator_config['types'][media_type]['update']:
        ...
        for f in good_files:
            ...
            generator_config['types'][media_type]['items'][md5_returned] = {"video_title": video_title, "path": f, "filename": filename, "duration_in_seconds": video_length_in_seconds}
            ...
            if generator_config['create_gifs']:
                ...
                fifth_gif_start = math.floor(video_length_in_seconds) - 180
                sixth_gif_start = math.floor(video_length_in_seconds) - 90
                ...
                if not os.path.exists("{}{}/{}.{}.gif".format(generator_config["gifs_path"],filename[:-4],filename, 7)):
                    ...
                    if generator_config['overwrite_gifs']:
                        print("OVERWRITING GIF7 at {}".format((seventh_gif_start)))
                        create_gifs_from_video(
        ...
        for f in weird_files:
            ...
            generator_config['types'][media_type]['weird_file_detail'][md5_returned] = {"video_title": video_title, "path": f, "filename": filename, "duration_in_seconds": video_length_in_seconds}
...
with open('rstv media list good files.csv', 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile, delimiter=',',
                            quoting=csv.QUOTE_MINIMAL)
    spamwriter.writerow(['Title','Media Type','Filename', 'Duration In Seconds','Full Path','md5','Issues'])
    for media_type in updated_types:
        for item in generator_config['types'][media_type]['items']:
            spamwriter.writerow([generator_config['types'][media_type]['items'][item]['video_title'], media_type, generator_config['types'][media_type]['items'][item]['filename'], generator_config['types'][media_type]['items'][item]['duration_in_seconds'],generator_config['types'][media_type]['items'][item]['path'], item])
            
with open('rstv media weird files only.csv', 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile, delimiter=',',
                            quoting=csv.QUOTE_MINIMAL)
    spamwriter.writerow(['Title','Media Type','Filename', 'Duration In Seconds','Full Path','md5','Issues'])
    for media_type in updated_types:
        for item in generator_config['types'][media_type]['weird_file_detail']:
            spamwriter.writerow([generator_config['types'][media_type]['weird_file_detail'][item]['video_title'], media_type, generator_config['types'][media_type]['weird_file_detail'][item]['filename'], generator_config['types'][media_type]['weird_file_detail'][item]['duration_in_seconds'],generator_config['types'][media_type]['weird_file_detail'][item]['path'], item, "Bad Metadata"])
            
with open('rstv media list combined.csv', 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile, delimiter=',',
                            quoting=csv.QUOTE_MINIMAL)
    spamwriter.writerow(['Title','Media Type','Filename', 'Duration In Seconds','Full Path','md5','Issues'])
    for media_type in updated_types:
...
print(f"Took {toc - tic:0.4f} seconds. All done.")
